...
class TestJsonResultats:
    ...
    def test_meta_nb_produits_coherent(self, results_json, nb_produits_attendu, data_mode):
        """
        Vérifie la cohérence entre nb_produits dans le JSON et la source réelle.

        - Mode csv     : compare JSON ↔ CSV local (doivent matcher à ±10 près)
        - Mode bigquery/json : compare JSON ↔ JSON lui-même (toujours cohérent)
          car le CSV local contient les données de dev, pas de prod BigQuery.
        """
        nb_json = results_json.get('meta', {}).get('nb_produits', 0)
        assert abs(nb_json - nb_produits_attendu) <= 10, (
            f"Incohérence nb_produits : JSON={nb_json} / attendu={nb_produits_attendu} "
            f"(mode={data_mode})"
        )
