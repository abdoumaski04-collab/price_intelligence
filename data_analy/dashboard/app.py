...
def hex_to_rgba(hex_color, alpha=0.08):
    """Convertit une couleur hex en rgba valide pour Plotly"""
    h = hex_color.lstrip('#')
    ...
    with col_r:
        ...
        for site in sites_sel:
            ...
            fig_kde.add_trace(go.Scatter(
                ...
                fill='tozeroy', fillcolor=hex_to_rgba(COLORS[site], 0.08),
            ))
...
with tab7:
    st.markdown('<div class="section-title">📄 Génération du Rapport PDF</div>',
        unsafe_allow_html=True)

    import subprocess, sys, base64

    BASE_APP = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    PDF_PATH  = os.path.join(BASE_APP, 'outputs', 'rapport_prix_intelligence.pdf')
    SCRIPT    = os.path.join(BASE_APP, 'generer_rapport.py')
    ...
    with col_info:
        st.markdown("""
        <div style="background:linear-gradient(135deg,#EFF6FF,#DBEAFE);
          border:1px solid #93C5FD;border-radius:12px;padding:1.2rem 1.5rem">
          <div style="font-weight:600;color:#1D4ED8;font-size:1rem;margin-bottom:0.8rem">
            📋 Contenu du rapport (7 pages)
          </div>
          <div style="font-size:0.85rem;color:#1E40AF;line-height:1.9">
            <b>Page 1</b> — Couverture + KPIs globaux<br>
            <b>Page 2</b> — Statistiques descriptives + Boxplot + Bar chart<br>
            <b>Page 3</b> — Intervalles de confiance 95% + Shapiro-Wilk + Kruskal-Wallis<br>
            <b>Page 4</b> — Power Analysis (Cohen's d) + Régression OLS<br>
            <b>Page 5</b> — Distribution KDE + Analyse promotions<br>
            <b>Page 6</b> — Évolution 30j + Price Velocity + Segmentation<br>
            <b>Page 7</b> — Corrélation Spearman + Conclusions
          </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_btn:
        ...
        # Vérifier si un rapport existant est disponible
        rapport_existe = os.path.exists(PDF_PATH)
        if rapport_existe:
            taille  = os.path.getsize(PDF_PATH) / 1024
            modifie = datetime.fromtimestamp(os.path.getmtime(PDF_PATH))
            st.markdown(f"""
            <div style="background:var(--color-background-success);
              border-radius:10px;padding:.8rem 1rem;margin-bottom:1rem;
              border:0.5px solid var(--color-border-tertiary)">
              <div style="font-size:.8rem;color:var(--color-text-success);font-weight:500">
                ✅ Rapport disponible
              </div>
              <div style="font-size:.8rem;color:var(--color-text-secondary);margin-top:3px">
                {taille:.0f} Ko — {modifie.strftime('%d/%m %H:%M')}
              </div>
            </div>
            """, unsafe_allow_html=True)
        else:
            ...
            <div style="background:var(--color-background-warning);
              border-radius:10px;padding:.8rem 1rem;margin-bottom:1rem;
              border:0.5px solid var(--color-border-tertiary)">
              <div style="font-size:.8rem;color:var(--color-text-warning);font-weight:500">
    ...
    # ── Boutons ───────────────────────────────────────────────
    col_gen, col_dl, col_spacer = st.columns([1, 1, 2])

    with col_gen:
        generer = st.button("🔄 Générer le rapport",
    ...
    with col_dl:
        if rapport_existe:
            ...
            st.download_button(
                ...
                data=pdf_bytes,
                file_name=f"rapport_prix_intelligence_{datetime.now().strftime('%Y%m%d')}.pdf",
    ...
    # ── Génération ────────────────────────────────────────────
    if generer:
        with st.spinner("⏳ Génération du rapport en cours (30 sec environ)..."):
            try:
                ...
                if result.returncode == 0:
                    ...
                    # Log de génération
                    with st.expander("📋 Détails de génération", expanded=False):
                    ...
                    if os.path.exists(PDF_PATH):
                        ...
                        with open(PDF_PATH, 'rb') as f:
                            pdf_bytes = f.read()
                        st.download_button(
                            label=f"⬇️ Télécharger le rapport ({taille:.0f} Ko)",
                            data=pdf_bytes,
                            file_name=f"rapport_prix_intelligence_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
                            mime="application/pdf",
                ...
                st.error("❌ Timeout dépassé (>120 sec). Vérifier les données.")
            except Exception as e:
                st.error(f"❌ Erreur : {e}")

    # ── Aperçu des métriques du rapport ───────────────────────
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="section-title">Métriques incluses dans le rapport</div>',
        unsafe_allow_html=True)

    m1, m2, m3, m4 = st.columns(4)
    with m1:
        st.metric("Produits", f"{results['meta']['nb_produits']:,}")
    with m2:
        reg = results.get('regression', {})
        st.metric("R² régression", f"{reg.get('r2_adj', 0):.4f}")
    with m3:
        alertes_list = results.get('alertes', [])
        st.metric("Alertes", len(alertes_list))
    with m4:
        ml = results.get('machine_learning', {})
        rf = ml.get('modeles', {}).get('random_forest', {})
        st.metric("R² ML (RF)", f"{rf.get('r2', 0):.4f}")

    # ── Tableau récap des tests statistiques ──────────────────
    st.markdown('<div class="section-title">Résumé des tests statistiques</div>',
        unsafe_allow_html=True)
    ...
    for site in ['ikea', 'jumia', 'kitea']:
        if site in shapiro:
            d = shapiro[site]
            tests_data.append({
                'Test'    : f'Shapiro-Wilk ({site})',
                'Stat.'   : f"W = {d['stat']:.4f}",
                'p-value' : f"{d['p_value']:.6f}",
                'Résultat': '❌ Non normale' if not d['normal'] else '✅ Normale',
            })
    kw_c = results.get('kruskal_categories', {})
    kw_s = results.get('kruskal_sites', {})
    if kw_c:
        tests_data.append({'Test': 'Kruskal-Wallis (catégories)',
            'Stat.': f"H = {kw_c['stat']:.2f}",
            'p-value': f"{kw_c['p_value']:.2e}",
            'Résultat': '✅ Significatif' if kw_c['p_value'] < 0.05 else '❌ Non sig.'})
    if kw_s:
        tests_data.append({'Test': 'Kruskal-Wallis (sites)',
            'Stat.': f"H = {kw_s['stat']:.2f}",
            'p-value': f"{kw_s['p_value']:.2e}",
            'Résultat': '✅ Significatif' if kw_s['p_value'] < 0.05 else '❌ Non sig.'})

    if tests_data:
        st.dataframe(pd.DataFrame(tests_data), hide_index=True,
            use_container_width=True)
