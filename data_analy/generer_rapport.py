...
def construire_pdf(df, df_h, results, charts, pdf_path):
    ...
    if power:
        ...
        for key, d in power.items():
            puissance  = d.get('puissance', d.get('puissance_actuelle', 0))
            n_req      = d.get('n_minimum', d.get('n_necessaire', '—'))
            n_act      = d.get('n_actuel', d.get('n_minimum', '—'))
            effet      = d.get('taille_effet', d.get('interpretation', '—'))
            conclusion = '✅ OK' if puissance >= 0.80 else '⚠️ Insuffisant'
            pw_rows.append([
                key.replace('_vs_', ' vs '),
                f"{d['cohens_d']:.4f}", effet,
                f"{puissance*100:.1f}%",
                f"{n_act:,}" if isinstance(n_act, int) else str(n_act),
                str(n_req),
                conclusion,
            ])
